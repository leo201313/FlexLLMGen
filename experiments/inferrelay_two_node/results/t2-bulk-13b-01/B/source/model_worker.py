...
class Channel:
    ...
    def calibrate(self,first):
        # Offset B-A bounded by nonnegative one-way delays; never assume RTT/2.
        intervals=[]
        for i in range(8):
            if first:
                t1=ms();self.send({'clock':i,'t1':t1});msg=self.recv();t4=ms()
                validate(msg,{'clock':i});intervals.append([msg['t3']-t4,msg['t2']-t1])
            else:
                msg=self.recv();t2=ms();validate(msg,{'clock':i})
                self.send({'clock':i,'t2':t2,'t3':ms()})
        if first:
            best=min(intervals,key=lambda x:x[1]-x[0]);result={'offset_last_minus_first_ms':best,'width_ms':best[1]-best[0],'samples':intervals}
            self.send({'calibration':result})
        else:result=self.recv()['calibration']
        return result
    ...
    def transfer(self,x,shape,request,step,group,sender):
        ...
        try:
            ...
            with torch.cuda.stream(self.stream):
                ...
                b.record()
            b.synchronize();stage_end=ms();self.weights.pause.clear()
            if sender:
...
def run_request(exe,channel,owners,ids,steps,request,reference=None,capture=False):
    ...
    calibration=channel.calibrate(exe.first) if channel else None
    if channel:channel.reset();channel.barrier('start-'+str(request));channel.reset()
    torch.cuda.synchronize()
    anchor=event();anchor_lo=ms();anchor.record();anchor.synchronize();anchor_hi=ms();origin=anchor_hi
    ...
    return dict(e2e_ms=elapsed,calibration=calibration,gpu_anchor_host_ms=[anchor_lo,anchor_hi],step_ms=step_times,capture_overhead_included=capture,errors=errors,correct=True,
