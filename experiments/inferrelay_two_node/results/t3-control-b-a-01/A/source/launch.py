...
def main():
    ...
    p.add_argument('--worker',choices=['activation','interference','model_worker'],default='activation')
    p.add_argument('--config',type=Path)
    p.add_argument('--repeats',type=int,default=20)
    p.add_argument('--timeout',type=int,default=600)
    args=p.parse_args()
    lock=open('/tmp/inferrelay-two-node-workers.lock','a+')
    try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    except BlockingIOError:raise RuntimeError('Another local two-node launcher is active; stop it or wait before starting another run')
    ...
    def start(node):
        ...
        argv=[PYTHON,'-u','-m','experiments.inferrelay_two_node.'+args.worker,'--node',node,'--role',role,
            '--model',args.model,'--host',host,'--out',str(out/node),'--run-id',run_id,'--repeats',str(args.repeats)]
        if args.quick: argv.append('--quick')
        if args.config: argv += ['--config',str(args.config.resolve())]
