...
class WeightSlots:
    ...
    def copy(self,index,g):
        ...
        pending=collections.deque();chunk_events=[]
        ...
        self.records.append(dict(index=index,group=g,slot=slot,bytes=source.numel()*2,host_submit_ms=ms()-t,copy_call_ms=call_ms,
            buffer_host_wait_ms=host_buffer_wait,a=a,b=b,w0=w0,w1=w1,chunks=chunk_events))
    ...
    def finish(self):
        ...
        for r in self.records:
            r['b'].synchronize()
            out.append({**{k:v for k,v in r.items() if k not in ['a','b','w0','w1','chunks']},
                ...
                'chunks':[[self.anchor.elapsed_time(a),self.anchor.elapsed_time(b)] for a,b in r['chunks']]})
        return out
