...
def schedule(groups, owners, requests=1, arrival_gap_ms=0, cycles=1, buffers=2,
    ...
    Activations explicitly compete with weights for destination H2D. Independent copies
    backfill free resource intervals, so an early reservation of a future activation does
    not artificially prevent the next weight prefetch. This is not an optimal oracle.
    """
    ...
    turns=[0,0]
    previous_copy=[0.,0.]
    intervals={resource:[] for resource in free}
    events=[]
    ends=[]
    first=[]
    copy_wait=0.
    ...
    def reserve(resource,ready,duration,kind,req,group=-1,slot=-1):
        start=ready
        for a,b in sorted(intervals[resource]):
            if start+duration<=a:
                break
            if start<b:
                start=b
        ...
        free[resource]=max(free[resource],end)
    ...
    for req in range(requests):
        ...
        for cycle in range(cycles):
            for j,((copy_ms,compute_ms,_),owner) in enumerate(zip(groups,owners)):
                ...
                loaded=reserve(f'h2d{owner}',max(arrival,slots[owner][slot],previous_copy[owner]),copy_ms*copy_penalty,
                               'weight_h2d',req,j,slot)
                previous_copy[owner]=loaded
                if prev_owner!=owner:
