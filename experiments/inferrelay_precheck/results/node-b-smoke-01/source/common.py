...
class Memory:
    ...
    def __exit__(self, *args):
        ...
        if host_stats:
            try:
                raw = host_stats()
                self.values['host_allocator_stats'] = raw
                # Keep process-lifetime reserved peaks separate from per-case GPU peaks.
                self.values['pinned_reserved_peak_bytes_process_lifetime'] = raw.get('reserved_bytes.peak')
                inconsistent = any(v < 0 for v in raw.values() if isinstance(v, (int,float)))
                self.values['host_allocator_negative_counter_observed'] = inconsistent
                if not inconsistent:
                    self.values['pinned_allocator_peak_bytes'] = raw.get('allocated_bytes.peak')
                self.values['pinned_allocator_peak_scope'] = 'process_lifetime_not_reset_per_case'
            except Exception:
                pass
